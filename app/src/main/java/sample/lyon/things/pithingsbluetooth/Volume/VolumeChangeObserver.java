...
public class VolumeChangeObserver {
    String TAG = VolumeChangeObserver.class.getName();
    ...
    private static VolumeChangeListener mVolumeChangeListener;
    ...
    public int getCurrentSystemVolume() {
        return mAudioManager != null ? mAudioManager.getStreamVolume(AudioManager.STREAM_SYSTEM) : -1;
    }
    ...
    private  class VolumeBroadcastReceiver extends BroadcastReceiver {
        String TAG =VolumeBroadcastReceiver.class.getName();
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (VOLUME_CHANGED_ACTION.equals(intent.getAction())) {
                if( (intent.getIntExtra(EXTRA_VOLUME_STREAM_TYPE, -1) == AudioManager.STREAM_MUSIC)){
                    Log.d("VolumeChangeObserver:"+TAG,"STREAM_MUSIC");
                ...
                }else if( (intent.getIntExtra(EXTRA_VOLUME_STREAM_TYPE, -1) == AudioManager.STREAM_SYSTEM)) {
                    Log.d("VolumeChangeObserver:"+TAG,"STREAM_SYSTEM");
                    VolumeChangeObserver observer = mObserverWeakReference.get();
                    if (observer != null) {
                        VolumeChangeListener listener = observer.getVolumeChangeListener();
                        if (listener != null) {
                            int volume = observer.getCurrentSystemVolume();
                            if (volume >= 0) {
                                listener.onVolumeChanged(volume);
                            }
                        }
                    ...
                    Log.d("VolumeChangeObserver:"+TAG,"Other ");
                    VolumeChangeObserver observer = mObserverWeakReference.get();
                    if (observer != null) {
                        VolumeChangeListener listener = observer.getVolumeChangeListener();
                        if (listener != null) {
